...
def insert_test_result(
    test_name: str, actual_result, expected_result, BI_DATASET_ID: str
):
...
def get_file_version(repo, commit, file_path):
    ...
    return get_schema_versions_from_string(
    ...
    for root, dirs, files in os.walk("temp_repo"):
        for file in files:
            if file.endswith(".xsd"):
                ...
                ):  # If the latest version is None, then the file does not have a version
                    errors.append(f"{file_path}: File [{file}] does not have a version")
                    continue

                if len(commits) > 1:
                    second_latest_commit = commits[1]
                    previous_version = get_file_version(
                        repo, second_latest_commit, file_path
                    )
                    if (
                        latest_version == previous_version
                    ):  # If the latest version is the same as the previous version, then the version has not changed
                        errors.append(
                            f"{file_path}: Schema version for file [{file}] has not changed!"
                        )
    ...
    for sibling_dir in os.listdir(parent_dir):
        ...
        for xsd_file in xsd_files:
            xml_file = os.path.join(
                xml_dir, os.path.basename(xsd_file).replace(".xsd", ".xml")
            )
        ...
        for xml_file in xml_files:
            xsd_file = os.path.join(
                xsd_dir, os.path.basename(xml_file).replace(".xml", ".xsd")
            )
